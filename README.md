...
推荐先买散件，不买整机套装。这样更容易和当前 `device/m5stack-stackchan` 固件对齐，也方便后面改舵机、供电和外壳。

| 配件 | 数量 | 建议关键词 | 说明 |
| --- | ---: | --- | --- |
| M5Stack CoreS3 | 1 | `M5Stack CoreS3 K128` | 首选主控。自带屏幕、Wi-Fi、扬声器、双麦克风、摄像头和电池，适合后续做语音和视觉。 |
| MG90S 180 度舵机 | 4 | `MG90S 180度 舵机` | 实际只用 2 个：左右转头、点头；多买 2 个备用。想更便宜可以换 `SG90 180度 舵机`。 |
| 5V 2A 舵机供电模块 | 1 | `5V 2A 降压模块`、`舵机供电模块` | 舵机不要直接吃 CoreS3 的 3.3V。舵机单独 5V 供电，并和 CoreS3 共地。 |
| Stack-chan 3D 打印外壳 | 1 套 | `Stack-chan 外壳`、`Stackchan 3D打印` | 没有打印机就找 3D 打印店。下单前确认适配 M5Stack CoreS3/Core2 和 SG90/MG90S 舵机。 |
| M2 螺丝包 | 1 | `M2 螺丝包`、`M2x4 M2x8` | 固定外壳、屏幕和舵机。可以买混装包。 |
| 杜邦线和转接线 | 1 批 | `杜邦线`、`Grove 转杜邦线`、`HY2.0 转杜邦` | 用来连接舵机信号线、5V 和 GND。 |
| USB-C 数据线 | 1 | `USB-C 数据线` | 要能传数据，用于烧录固件和串口调试。 |

可后买的配件：

- I2S 功放和更大的喇叭：CoreS3 自带扬声器，第一版先不用外接。
- 外接麦克风：第一版可以先用 Mac、树莓派或浏览器收音。
- M5Stack Servo Module：当前固件先用 GPIO PWM 直接控制两个舵机；如果后面想接更多舵机或整理供电，再换 I2C 舵机驱动板。
- 电池和充电模块：第一版建议先 USB-C 供电，稳定后再做移动电源或内置电池。

下单前问店家确认：

1. 外壳是否适配 `M5Stack CoreS3`。
2. 舵机仓是否适配 `SG90/MG90S` 尺寸。
3. 是否包含舵机固定螺丝和连接件。
4. 是否需要自己焊接。
5. 有没有装配图或接线图。
